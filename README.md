...
Lanes are at least 20 columns and grow to fill the row. Extra lanes wrap to the next row and scroll vertically into view with h/l. A lane with more cards than fit shows a vertical scrollbar.
...
| space | Select |
| f | Filter the focused lane (arrows move among matches; j/k still type) |
